...
describe('PlaceService', () => {
  ...
  it('moveGoods should PATCH goods order', async () => {
    const goods = [{ id: 1, name: 'Beer' }] as any;

    const promise = service.moveGoods(12, goods);
    const req = httpMock.expectOne(API_URL + 'places/12/goods/move');
    expect(req.request.method).toBe('PATCH');
    expect(req.request.body).toEqual(goods);
    req.flush(null);
